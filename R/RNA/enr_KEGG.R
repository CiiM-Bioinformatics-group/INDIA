...
enrich <- function(genes, thresh.incl=5, n.show=5) {
  ...
  enr.KEGG <- enrichKEGG(gene = genes,
                         organism = 'hsa',
                         pAdjustMethod = "BH",
                         pvalueCutoff = 1,
                         qvalueCutoff = 0.05)
...
write.xlsx(x = up, file = 'output/enrKEGG_up_stimulations.xlsx')
write.xlsx(x = down, file = 'output/enrKEGG_down_stimulations.xlsx')
...
pdf('output/enrKEGG_up.pdf', width = 12, height = 10)
...
pdf('output/enrKEGG_down.pdf', width = 12, height = 10)
print(ggplot(data = df) +
        geom_point(aes(x = Time, y = Description, size = GeneRatio2, color = p.adjust)) +
        DOSE::theme_dose() +
        theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45),
